...
# 도구들의 성숙도 값을 0.25 단위로 조정 (예시)
for tool in tools.values():
    for subcategory in subcategories:
        if subcategory in tool:
            tool[subcategory] = round(tool[subcategory] * 4) / 4  # 0.25 단위로 반올림
        ...
        if category_vars[data['category']].get():
            for subcategory in subcategories:
                if subcategory_vars[subcategory].get() and subcategory in data:
                    ...
                    # 가장 가까운 0.25 단위 성숙도 레벨 찾기
                    closest_maturity = min(maturity_levels, key=lambda x: abs(float(x) - maturity))
                    if maturity_vars[closest_maturity].get():
        ...
        for cat_index, category in enumerate(categories):
            ...
            if category in grouped_tools and subcategory in grouped_tools[category]:
                for i, (tool, maturity) in enumerate(grouped_tools[category][subcategory]):
                    ...
                    ax.plot(x, y, 'o', color='gray', markersize=2.5)
                    ax.annotate(tool, (x, y + offset), xytext=(15, 15), textcoords='offset points', 
                                ha='left', va='bottom',
                                bbox=dict(facecolor='none', edgecolor='black', alpha=0.3, boxstyle='round,pad=0.2'),
                                arrowprops=dict(arrowstyle='->', connectionstyle='arc3,rad=0.2', color='gray'),
                                rotation=30)  # 레이블 회전 각도를 30도로 변경
