    ...
    def get_article_links(self):
        try:
            ...
            for section in sections:
                ...
                # Updated selectors for Reuters
                article_containers = soup.find_all(['article', 'div'], 
                    class_=['story', 'article', 'media-story'])
                
                for container in article_containers:
                    links_found = container.find_all('a', href=True)
                    for link in links_found:
                        href = link['href']
                        if href.startswith('/'):
                            full_url = f"https://www.reuters.com{href}"
                            links.add(full_url)
                        elif href.startswith('https://www.reuters.com'):
                            links.add(href)

                logging.info(f"Found {len(links)} Reuters links in {section}")
...
class YTNScraper(threading.Thread):
    def __init__(self, queue, status_queue, db):
        ...
        self.db = db
        self.running = True
        logging.info("YTN Scraper initialized")

    def get_article_links(self):
        try:
            links = set()
            
            # Direct link to international news section
            url = f"{self.base_url}/news/list.php?mcd=0104"
            
            logging.info(f"Fetching links from YTN {url}")
            self.status_queue.put(("status", f"Checking YTN international news..."))
            
            response = requests.get(url, headers=self.headers)
            response.encoding = 'utf-8'
            soup = BeautifulSoup(response.text, 'html.parser')
            
            # Find all news items using the correct class
            news_items = soup.select('.news_list .title a')
            ...
            for item in news_items:
                ...
                if href:
                    ...
                    links.add(full_url)
                    logging.debug(f"Found YTN link: {full_url}")

            links = list(links)
            logging.info(f"Total YTN links found: {len(links)}")
            print("YTN Links:", links)  # Debug print
            return links

        except Exception as e:
            logging.error(f"Error getting YTN article links: {e}")
            print(f"Error in YTN scraper: {e}")
            return []
        
    def scrape_article(self, url):
        try:
            logging.info(f"Scraping YTN article: {url}")
            print(f"Attempting to scrape: {url}")  # Debug print
            
            response = requests.get(url, headers=self.headers)
            response.encoding = 'utf-8'
            soup = BeautifulSoup(response.text, 'html.parser')
            
            # Title extraction - using YTN's actual title selector
            title = ""
            title_div = soup.select_one('.article_tit')  # Update selector for title
            if title_div:
                title = title_div.text.strip()
                print(f"Found title: {title}")
            
            # Content extraction - using YTN's actual content selector
            content = ""
            content_div = soup.select_one('.article_txt')  # Update selector for content
            if content_div:
                # Remove unnecessary elements
                for unwanted in content_div.select('.adrs, .copyright'):
                    unwanted.decompose()
                
                # Get all paragraphs
                content = ' '.join([p.text.strip() for p in content_div.find_all('p') if p.text.strip()])
            ...
            if title and content:
                ...
                logging.warning(f"Incomplete YTN article - Title exists: {bool(title)}, Content exists: {bool(content)}")
                print(f"Failed to get complete article - Title: {bool(title)}, Content: {bool(content)}")
                return None

        except Exception as e:
            logging.error(f"Error scraping YTN article {url}: {e}")
    ...
    def run(self):
        while self.running:
            try:
                msg = "Starting to scrape YTN news..."
                logging.info(msg)
                self.status_queue.put(("status", msg))
                ...
                for i, link in enumerate(links, 1):
                    ...
                    status_msg = f"Scraping YTN article {i}/{len(links)}..."
                    self.status_queue.put(("status", status_msg))
                ...
                logging.error(f"Error in YTN scraping cycle: {e}")
                time.sleep(60)

    def stop(self):
        logging.info("Stopping YTN scraper")
        self.running = False
...
class NewsApp(tk.Tk):
    ...
    def load_config(self):
        default_config = {
            "news_sources": [
                ...
                {
                    "name": "YTN",
                    "base_url": "https://ytn.co.kr",
                    "sections": [
                        "/news/list.php?mcd=0101",  # Politics
                        "/news/list.php?mcd=0102"  # Economy
                    ]
                }
            ],
            "update_interval": 300,
        ...
        try:
            ...
            logging.warning(f"Config file issue: {e}. Creating new config file.")
    ...
    def create_menu(self):
        ...
        for source in self.settings["news_sources"]:
            ...
            # Fix the lambda to create a new function for each source
            sources_menu.add_checkbutton(
                label=source["name"],
                variable=var,
                command=lambda s=source: self.toggle_source(s)  # Add s=source to capture current value
            )
    ...
    def start_source_scraper(self, source):
        ...
        if source["name"] not in self.active_scrapers:
            ...
            elif source["name"] == "YTN":
                scraper = YTNScraper(self.queue, self.status_queue, self.db)
            else:
