...
category_choices = ['* (All Categories)'] + categories
category_menu = ttk.Combobox(menu_frame, textvariable=category_var, values=category_choices)
category_menu.pack(side=tk.LEFT, padx=5)
category_menu.set('* (All Categories)')

def plot_landscape(selected_category='* (All Categories)'):
    ax.clear()
    ...
    for tool, data in tools.items():
        if selected_category == '* (All Categories)' or data['category'] == selected_category:
            grouped_tools[data['category']].append((tool, data['maturity']))

    # Sort tools by maturity and get top 3 (or all if specific category is selected)
...
def on_category_select(*args):
    ...
    plot_landscape(selected_category)
...
info_label = ttk.Label(info_frame, text="Select a category from the menu to see all tools in that category", wraplength=1380)
info_label.pack()
