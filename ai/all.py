...
for (category, maturity), tool_list in grouped_tools.items():
    ...
    for i, tool in enumerate(unique_tools):
        offset = (i - (len(unique_tools) - 1) / 2) * 0.2
...
def toggle_magnify():
    if magnify_button.config('text')[-1] == 'Enable Magnify':
        magnify_button.config(text='Disable Magnify')
        canvas.mpl_connect('motion_notify_event', on_hover)
    else:
        magnify_button.config(text='Enable Magnify')
...
def on_hover(event):
    if event.inaxes == ax:
        ...
        for (category, maturity), tool_list in grouped_tools.items():
            x = maturity
            y = categories.index(category)
            if event.xdata - 0.5 <= x <= event.xdata + 0.5 and event.ydata - 0.5 <= y <= event.ydata + 0.5:
                ax.plot(x, y, 'ro', markersize=20)
                unique_tools = list(dict.fromkeys(tool_list))
                for i, tool in enumerate(unique_tools):
                    offset = (i - (len(unique_tools) - 1) / 2) * 0.1
                    ax.annotate(tool, (x, y + offset), xytext=(40, 0), textcoords='offset points', ha='left', va='center',
                                bbox=dict(facecolor='white', edgecolor='black', boxstyle='round,pad=0.2'),
                                arrowprops=dict(arrowstyle='->', connectionstyle='arc3,rad=0'))
        canvas.draw()

# Add magnify button
magnify_button = tk.Button(root, text="Enable Magnify", command=toggle_magnify)
magnify_button.pack()

# Info display
info_frame = ttk.Frame(root)
info_frame.pack(fill=tk.X, padx=10, pady=10)
info_label = ttk.Label(info_frame, text="Use the magnify button to zoom in on areas", wraplength=1380)
info_label.pack()
